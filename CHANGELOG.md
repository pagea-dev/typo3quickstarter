...
- `--release=9` and `--release=10` install the two older LTS lines, for getting an extension in front of a running instance of the version it was written for before moving it forward. Both run on PHP 7.4, are set up with the legacy `typo3cms install:setup` like `--release=11` already was, and have their `PackageStates.php` regenerated before the extension setup, so extensions added with `--require`/`--extension` actually come up active. Three things the old lines need on top are handled along the way: TYPO3 Console's Composer plugin is added to the distributions' outdated `allow-plugins` list, which Composer 2.2+ otherwise aborts on; TYPO3 9's site configuration gets its base URL written after the fact, since `--site-base-url` only exists from TYPO3 Console 6 on and TYPO3 9 leaves an unusable `base: ht/` behind without it; and the `--with-git` extension kickstarter keeps needing TYPO3 12+. The debug settings every instance gets now also reach 9, 10 and 11, which have no `settings.php` for the script to write into - see [docs/versions.md](docs/versions.md) ([#21](https://github.com/pagea-dev/typo3quickstarter/issues/21)).
- phpMyAdmin is now part of every instance, so a look into the database no longer means installing DDEV's add-on and restarting the project first. It is installed between `ddev config` and the first `ddev start`, comes up with everything else, and opens straight into the database without a login screen - the URL is in the final summary and in `typo3-credentials.txt`, and `ddev phpmyadmin` opens the same thing from inside the project. `--no-pma` leaves it out, and a failed download never breaks a setup - see [docs/phpmyadmin.md](docs/phpmyadmin.md) ([#20](https://github.com/pagea-dev/typo3quickstarter/issues/20)).
