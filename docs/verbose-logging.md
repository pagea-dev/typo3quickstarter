...
- **Treat it like `typo3-credentials.txt`.** The final summary (admin username/password) gets logged too, so `verbose.log` gets the same `chmod 600` and a `.gitignore` entry automatically.
